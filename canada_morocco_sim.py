...
def simulate():
    ...
    mar_wins_2h = m2 > c2
    can_wins_2h = c2 > m2
    can_wins_both = can_wins_ht and can_wins_2h and ct > mt
    mar_wins_both = mar_wins_ht and mar_wins_2h and mt > ct
    mar_wins_one_half = mar_wins_ht or mar_wins_2h
    tg_2h = c2 + m2

    return {
        'r90': r90, 'qual': qual, 'ct': ct, 'mt': mt, 'tg': tg,
        'c1': c1, 'm1': m1, 'c2': c2, 'm2': m2, 'et': et,
        'btts': ct > 0 and mt > 0,
        'u25': tg <= 2.5,
        'u35': tg <= 3.5,
        'u45': tg <= 4.5,
        ...
        'can_not_both': not can_wins_both,
        'mar_wins_one': mar_wins_one_half,
        'mar_dc_u35': r90 in ('M', 'D') and tg <= 3.5,
        'can_dc_u35': r90 in ('C', 'D') and tg <= 3.5,
        'can_dc_u45': r90 in ('C', 'D') and tg <= 4.5,
        'cm_dc_u45': r90 in ('C', 'M') and tg <= 4.5,
        'mar_dc_o15': r90 in ('M', 'D') and tg >= 2,
        'mar_dc': r90 in ('M', 'D'),
        'can_dc': r90 in ('C', 'D'),
        'mar_u15': mt <= 1.5,
        'can_u05': ct == 0,
...
# Betclic odds from screenshots (04/07)
candidates = [
    ('Under 1.5 golos 1.ª parte', 1.31, lambda r: r['u15_1h']),
    ('Under 1.5 golos 2.ª parte', 1.42, lambda r: r['u15_2h']),
    ('MAR ganha uma das partes (Sim)', 1.34, lambda r: r['mar_wins_one']),
    ('MAR/Empate & Under 3.5', 1.38, lambda r: r['mar_dc_u35']),
    ('MAR/Empate & Over 1.5', 1.48, lambda r: r['mar_dc_o15']),
    ('CAN/Empate & Under 4.5', 1.32, lambda r: r['can_dc_u45']),
    ('CAN/MAR & Under 4.5', 1.32, lambda r: r['cm_dc_u45']),
    ('CAN +2 handicap', 1.31, lambda r: r['can_hcp2']),
    ('Over 0.5 golos 1.ª parte', 1.40, lambda r: r['o05_1h']),
    ('Under 2.5 golos', 1.58, lambda r: r['u25']),
    ('BTTS Nao', 1.75, lambda r: not r['btts']),
    ('MAR nao vence ambas (Nao)', 1.11, lambda r: r['mar_not_both']),
    ('CAN nao vence ambas (Nao)', 1.01, lambda r: r['can_not_both']),
    ('Prolongamento Nao', 1.24, lambda r: r['no_et']),
]
...
for name, odd, fn in candidates:
    if 1.30 <= odd <= 1.50:
        continue
    p = pct(fn)
    print(f"  {name:36s} @{odd:.2f} M:{p:5.1f}%")

rows.sort(reverse=True)
if rows:
    best = rows[0]
    print(f"\n>>> MELHOR NA GAMA: {best[1]} @{best[2]:.2f} — {best[0]:.1f}%")
